...
def process_mail_command(conn, data, msg):
    if data is not None:
        ...
        if cmd == 'QUIT': process_quit(conn,msg='Goodbye')
        ...
        if cmd == 'DATA': send_response(conn, '354', msg = 'End data with .')
        return cmd
...
def process_envelope(conn, msg, cmd, param):
    if cmd == 'MAIL':
        m=re.search('FROM: <(.+)>', param) #search with pattern regex
        msg['from'] = m.group(1)

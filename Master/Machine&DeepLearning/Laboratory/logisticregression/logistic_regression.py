...
def loss(y_true, y_pred):
    ...
    """
    first_term = y_true * np.log(y_pred + eps)
    second_term = (1 - y_true) * np.log(1 - y_pred + eps)
    return - np.mean(first_term + second_term, axis = 0)
...
def dloss_dw(y_true, y_pred, X):
    ...
    derivative = -(np.matmul(X.T,(y_true - y_pred))) / N
    """
...
class LogisticRegression:
    ...
    def predict(self, X):
        ...
        score = np.round(sigmoid(dot_product)) # Evenly round to the given number of decimals.

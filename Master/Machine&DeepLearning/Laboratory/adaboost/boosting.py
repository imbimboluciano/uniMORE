...
class WeakClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray):
        ...
        self._threshold = np.random.uniform(low = min(X[:,self._dim]), high=max(X[:,self._dim]))

        """ pick a random verse (see np.random.choice)
            case a) feature >= _threshold ==>> then predict 1
            case b) feature >= _threshold ==>> then predict -1 """
        self._label_above_split = np.random.choice(possible_labels) # or 1 or -1
...
class AdaBoostClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, verbose: bool = False):
        ...
        sample_weights = np.ones(shape=n) / n # np.random.uniform(a=rnage(0,n))
        ...
        for l in range(self.n_learners):
            ...
            sample_weights[cur_idx[y_pred != cur_Y]] = sample_weights[cur_idx[y_pred != cur_Y]] * np.exp(alpha)
            sample_weights[cur_idx[y_pred == cur_Y]] = sample_weights[cur_idx[y_pred == cur_Y]] * np.exp(-alpha)
            sample_weights = sample_weights / np.sum(sample_weights) # Z factor to normalize
    ...
    def predict(self, X: np.ndarray):
        ...
        predictions_learners = np.zeros(shape=(num_samples,self.n_learners))

        for index, learner in enumerate(self.learners):
            predictions_learners[:,index] = learner.predict(X)
            
        y_pred = np.sign(np.sum(predictions_learners * self.alphas, axis=1)) # taken from solution, why axis 1? Because I have to sum value on columns

...
def people_visualization(X,y):
    ...
    # plt.grid(False)  # Remove this line or change to plt.grid(False) if needed
...
def people_visualize_prediction(X,y,y_pred):
    ...
    for i in range(num_row):
        for j in range(num_col):
            ...
            # Option 1: Remove the grid call
            # Option 2: Use older syntax
            subplots[i,j].grid(False)
            # Option 3: Set grid visibility directly
            # subplots[i,j].xaxis.grid(False)
            # subplots[i,j].yaxis.grid(False)

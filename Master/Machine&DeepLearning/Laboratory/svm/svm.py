...
class SVM:
    ...
    def loss(self, y_true, y_pred):
        ...
        """
        first_term = 0.5 * self._lambda * np.linalg.norm(self._w)
        second_term = np.mean(np.maximum(np.zeros(shape=y_true.shape[0]),y_true * y_pred)) # y_pred = dot product between x and w
    ...
    def fit_gd(self, X, Y, verbose=False):
        ...
        for e in range(1, self._n_epochs+1):
            for j in range(n_samples):
                t = t + 1
                step_size = 1 / (t * self._lambda)
                if Y[j] * (self._w @ X[j,:]):
                    self._w = (1 - step_size * self._lambda) * self._w + step_size * Y[j] * X[j,:]
                else:
                    self._w = (1 - step_size * self._lambda) * self._w
    ...
    def predict(self, X):
        ...
        """
        return np.where(np.dot(X, self._w) > 0.0, self._original_labels[1], self._original_labels[0]) # copied from solution
